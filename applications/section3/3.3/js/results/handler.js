$(document).ready(function() {
	...
	switch (true) {
		case (endur >= 17) : var endurstatus = '(Ослабленная деятельность ССС)'; break;
		case (endur >= 15 && endur <= 16.99) : var endurstatus = '(Нормальная деятельность ССС)'; break;
	...
	var onestagepercent = ((pulse / 100) * onestage).toFixed();
	...
	//var bobs = ((adpp - adp) / (pulseop - pulse)).toFixed(1);

	var belg = (((parseInt(pulsebb) + parseInt(pulseba) + parseInt(pulsebaa)) / 6 - 33) / 10).toFixed(1);
	...
	$('#mname').append(mname);
	$('#date').append(date);
	$('#kek').append(kek + ' усл. ед. ' + kekstatus);
	$('#endur').append(endur + ' усл. ед. ' + endurstatus);
	$('#pdp').append(pdp + ' усл. ед. ' + pdpstatus);
	$('#robin').append(robin + ' усл. ед. ' + robinstatus);
	$('#onestage').append(onestagepercent + '  усл. ед. ' + onestagestatus);
	$('#pulse').append(pulse + ' уд/мин');
	$('#pulseop').append(pulseop + ' уд/мин');
	$('#sistp').append(sistp + ' мм. рт. ст.');
	$('#diap').append(diap + ' мм. рт. ст.');
	$('#pulsebb').append(pulsebb + ' уд/мин');
	$('#pulseba').append(pulseba + ' уд/мин');
	$('#pulsebaa').append(pulsebaa + ' уд/мин');
	//$('#bobs').append(bobs);
	$('#belg').append(belg + ' ' + belgstatus);
